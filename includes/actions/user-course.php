...
class AutomatorWP_LearnPress_User_Course extends AutomatorWP_Integration_Action {
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Enroll user from a course', 'automatorwp-learnpress' ),
            'select_option'     => __( 'Enroll user from <strong>a course</strong>', 'automatorwp-learnpress' ),
            /* translators: %1$s: Post title. */
            'edit_label'        => sprintf( __( 'Enroll user to %1$s', 'automatorwp-learnpress' ), '{post}' ),
            /* translators: %1$s: Post title. */
            'log_label'         => sprintf( __( 'Enroll user to %1$s', 'automatorwp-learnpress' ), '{post}' ),
            'options'           => array(
                'post' => automatorwp_utilities_post_option( array(
                    'name'              => __( 'Course:', 'automatorwp-learnpress' ),
                    'option_none_label' => __( 'all courses', 'automatorwp-learnpress' ),
                    'option_custom'         => true,
                    'option_custom_desc'    => __( 'Course ID', 'automatorwp-learnpress' ),
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        foreach( $courses as $course_id ) {
            ...
            if ( ! $course->is_free() ) {
                // Create a new order
                $order = new LP_Order();
                $order->set_customer_note( __( 'Order created by AutomatorWP', 'automatorwp-learnpress' ) );
